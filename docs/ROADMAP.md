...
| **HIER4A-PATCH2**          | Compare accepted Flat Directional bands with automatic one-level nested parent/child containment derived from canonical folder ancestry.                                            | **Complete — one-level Nested approved as the reviewed default; Flat remains session-only comparison evidence.**                                                                |
...
| **HIER4B**                 | Soft Folder Clusters with a nested displayed folder hierarchy, ancestor pass-through compression, composed context menus, area-targetable nested guides, and four-side File ports.  | **Merged implementation — accepted folder behavior and root-neutral folder force are on `main`; Soft macro-layout adoption remains undecided.**                                 |
| **HIER4B-PATCH1**          | Make Adaptive Compass respond to Soft's two-dimensional external-reference geometry without changing Directional Bands.                                                             | **Implemented for QA — Adaptive Compass compatibility awaits graphical approval.**                                                                                              |
| **HIER4B-PATCH2**          | Keep truthful root source-folder metadata while excluding it from Soft folder-attraction centroids and force groups.                                                                | **Complete — merged on `main`; FIX5 extends the exclusion to all visual/structural grouping.**                                                                                  |
| **HIER4B-SPACING**         | Compare direct-folder scope and normalized 1/3 vs 1/4 ancestor pull, then spread complete Soft modules radially over one fixed structural layout.                                   | **FIX5 candidate in PR #106 — Focus-neutral grouping plus hard retained Nested hierarchy containment and named-folder coverage; native graphical QA is required before merge.** |
| **HIER4B-FIX6**            | Harden retained Nested unity against sparse child envelopes and keep failed File-reroot presentation generation-coherent.                                                           | **Candidate in PR #106 — automated gates pass; optimized native real-vault QA is required before merge.**                                                                       |
| **FOCUS-HIERARCHY-UX1**    | Repair prepared-graph retention during File reroot; add session-only Heading/Block presentation subfocus and horizontal Folder/Parent labels.                                       | **Candidate in PR #106 — automated and native-build validation required; do not merge before founder graphical QA.**                                                            |
| **HIERDISC1**              | Add shared selective Heading-subtree visibility plus a unified Focus Explorer with projection-scoped Files and canonical Headings for Classic and Modular Focus Hierarchy.          | **FIX3 implemented for QA — uniform neighborhood depth plus independent session-only Heading-tree collapse; founder graphical approval gates merge.**                           |
| **HIERSTAB1**              | Preserve spatial memory across eligible Focus Heading/Block disclosure changes with an ephemeral validated transition prior, bounded local repair, and deterministic cold fallback. | **Implemented for QA — automated gates and optimized native graphical review required; PR must remain unmerged pending founder approval.**                                      |
| **HIER4B-UNIFIED-REGIONS** | Compare split-when-needed ancestor wrappers with a policy that spatially unifies displayed ancestor regions.                                                                        | **Superseded by HIER4B-SPACING-FIX5 hard retained Nested containment.**                                                                                                         |
| **MODULAR-CONTEXT1**       | Extend Modular File/folder context menus with Network-parity Focus, Inspect, Hide File, Hide folder, and other shared actions.                                                      | **After HIER4B folder semantics.**                                                                                                                                              |
...
| **PHYSICS1 — Continuous simulation lifecycle**      | Lazy worker ownership, real temporary hard constraints, whole-graph reaction, Pull-aware cooling, and camera-neutral frame adoption.                             | **Hardened for Focus ≤100 and All ≤300 release boundaries; native All-300 evidence pending.** |
| **MOVE1 — Temporary physical File movement**        | Direct manipulation that ends on release and returns the File to automatic physical behavior.                                                                    | **Implementation ready at Focus ≤100 / All ≤300; native acceptance pending.**                 |
...
| **CONVERGENCE1C** | Duration-independent Global folder-macro convergence design and production integration.                                      | **Complete.** |
...
SPATIAL2 static dynamic convergence remains separate. PHYSICS1 supplies the
continuous runtime that MOVE1B activates only for a real temporary File move.
