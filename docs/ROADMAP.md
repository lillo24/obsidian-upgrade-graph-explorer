...
| **HIER4B**                 | Soft Folder Clusters with a nested displayed folder hierarchy, derived singleton compression, composed context menus, area-targetable nested guides, and four-side File ports. | **Merged implementation — accepted folder behavior and root-neutral folder force are on `main`; Soft macro-layout adoption remains undecided.**                                                                                             |
...
| **HIER4B-SPACING**         | Compare direct-folder scope and normalized 1/3 vs 1/4 ancestor pull, then spread complete Soft modules radially over one fixed structural layout.                              | **FIX3B candidate in PR #106 — structural immediate-folder packing proves rigid radial spacing safe over the full slider domain; native graphical QA is required before merge. Dense-graph Folder-strength continuity remains later work.** |
| **HIER4B-UNIFIED-REGIONS** | Compare current split-when-needed guides with a layout policy that prioritizes spatially unified displayed folder regions.                                                     | **Later experiment — excluded from PATCH1.**                                                                                                                                                                                                |
