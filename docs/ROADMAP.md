...
| **KG11 — Tauri local-vault workflow**                        | Desktop folder access and watching through a narrow source-provider adapter.                                                                       | **In progress** — KG11A and KG11B1 watch/change planning are complete; KG11B2 live application and resync are next. |
...
KG11B2 is next: it will serialize plan application through KG10, persist the
resulting identity/report state, preserve the live view, and execute full resync
for out-of-sync conditions. KG12 will use the KG5 harness
