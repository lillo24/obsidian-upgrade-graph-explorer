...
const spacings = [0, 50, 100] as const;
...
function rootNeutralFolderAttractionGroups(
  ...
  for (const [fileId, memberships] of focusSchematicSoftFolderScopeMemberships(
    ...
    if (fileId === input.model.rootModuleId) continue;
    for (const { folderKey } of memberships) {
  ...
  return Object.fromEntries(
    [...groups]
      ...
      .map(([folderKey, members]) => [folderKey, members.sort()]),
  );
}
    ...
    orderPolicies.map((policy) => [
      ...
      Object.fromEntries(
        internalVariants.map((internal) => [
          internal,
          Object.fromEntries(
            (policy === 'crossing-optimized' && internal === 'adaptive-compass'
              ? spacings
              : [50]
            ).map((spacing) => [
              spacing,
                  ...
                  return [
                    strength,
                    {
                      ...
                      folderAttraction: {
                        ...
                        rootExcluded: true,
                        enabled: attempt.evidence.folderInfluenceEnabled,
                        eligibleGroupCount:
                          attempt.evidence.runtime.repeatedFolderCount,
                        eligibleMemberCount:
                          attempt.evidence.metrics.repeatedFolderModuleCount,
                        groups: folderAttractionGroups,
                      },
                    },
                  ];
                }),
...
<label>Soft spacing<select id="spacing">${spacings.map((value) => `<option${value === 50 ? ' selected' : ''}>${value}</option>`).join('')}</select></label>
...
${[
  ...
  ['centroids', 'Eligible attraction centroids (root excluded)', true],
  ...
  .join('')}
</div></div><p class="legend">Spacing 0 preserves the old baseline; 50 is the evidence-selected default; 100 is the validated spacious anchor. Anchor comparison uses the production Adaptive Compass + crossing-optimized configuration; development comparator combinations remain at spacing 50. SC16 and SC26–SC28 show all five strengths side by side. Folder hulls may include the root; attraction centroids use eligible non-root Files only. Directional Bands is the unchanged HIER4A reference.</p></div>
...
<div class="decision"><b>Graphical review gate</b><br>Inspect spacing 0, 50, and 100 on the priority cases, verify root-neutral force on SC26–SC28 and Adaptive Compass on AC-S1–AC-S8, then tune 0/25/50/75/100 in the native Sandbox and report the preferred numeric spacing. Soft macro-layout adoption remains a separate decision.</div></main>
...
q('scenario').addEventListener('change',()=>{revisions();render()});for(const id of controls.slice(1))q(id).addEventListener('change',render);revisions();render();</script></body></html>`;
  ...
  await writeFile(
    ...
    'HIER4B-SPACING development-only Soft Compass and root-neutral graphical QA. Compare spacing 0, 50, and 100 here, then tune 0/25/50/75/100 in the native Sandbox. Directional settings are unchanged.\n',
    'utf8',
  );
  return indexPath;
}
