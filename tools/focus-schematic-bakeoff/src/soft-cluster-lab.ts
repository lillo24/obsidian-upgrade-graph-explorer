...
<title>HIER4B PATCH1 Soft Compass + Root-Neutral Lab</title>
<style>
...
</style></head><body><div class="top"><h1>HIER4B PATCH1 Soft Compass + Root-Neutral Lab</h1><div class="controls">
...
${[
  ...
  .join('')}
</div></div><p class="legend">SC16 and SC26–SC28 show all five strengths side by side. Folder hulls show visible exact-folder membership and may include the root. Attraction centroids use eligible non-root Files only. Directional Bands is the unchanged HIER4A production reference.</p></div>
...
<div class="decision"><b>Graphical review gate</b><br>Inspect SC26–SC28 for root-neutral folder force, then compare Adaptive Compass with Vertical Spine in AC-S1–AC-S8. The gold root remains a visible folder member but is never an attraction member.<br>ADOPT_SOFT_FOLDER_CLUSTERS · SOFT_CLUSTERS_REQUIRE_TUNING · KEEP_DIRECTIONAL_BANDS_ONLY · SOFT_CLUSTERS_REQUIRE_REDESIGN</div></main>
...
q('scenario').addEventListener('change',()=>{revisions();render()});for(const id of controls.slice(1))q(id).addEventListener('change',render);revisions();render();</script></body></html>`;
  ...
  await writeFile(
    ...
    'HIER4B-PATCH1 development-only Soft Compass graphical QA, updated for root-neutral Soft folder force. Directional settings are unchanged.\n',
    'utf8',
  );
  return indexPath;
}
