...
Schema-v4 data lives at `argument-workspace/library-v4.json` under
application-local data. It is independent of the selected vault, graph view
state, and workspace identity catalog. When v4 is absent, load prefers a valid
`library-v3.json`, then `library-v2.json`, and then `library-v1.json`, performs
the core deterministic migration, writes v4 through a temporary sibling, and keeps the old file as a
...
confirmed v4 file. A failure removes the temporary file where possible and
