...
Schema-v7 data lives at `argument-workspace/library-v7.json` under
application-local data. It is independent of the selected vault, graph view
state, and workspace identity catalog. When v7 is absent, load prefers a valid
`library-v6.json`, then v5, v4, v3, v2, and v1, performs the core deterministic migration, writes v7 through
...
confirmed v7 file. A failure removes the temporary file where possible and
