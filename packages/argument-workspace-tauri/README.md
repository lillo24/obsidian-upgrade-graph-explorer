...
Schema-v5 data lives at `argument-workspace/library-v5.json` under
application-local data. It is independent of the selected vault, graph view
state, and workspace identity catalog. When v5 is absent, load prefers a valid
`library-v4.json`, then `library-v3.json`, `library-v2.json`, and
`library-v1.json`, performs the core deterministic migration, writes v5 through
...
confirmed v5 file. A failure removes the temporary file where possible and
