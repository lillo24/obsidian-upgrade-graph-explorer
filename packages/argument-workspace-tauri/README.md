...
Schema-v9 data lives at `argument-workspace/library-v9.json` under
application-local data. It is independent of the selected vault, graph view
state, and workspace identity catalog. When v9 is absent, load prefers a valid
`library-v8.json`, then v7, v6, v5, v4, v3, v2, and v1, performs the core deterministic migration, writes v9 through
...
confirmed v9 file. A failure removes the temporary file where possible and
