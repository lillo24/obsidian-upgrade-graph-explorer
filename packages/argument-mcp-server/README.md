...
The server is read-only. It reloads and validates `library-v2.json` for every
...
- Windows: `%LOCALAPPDATA%\com.icarus.graph-explorer\argument-workspace\library-v2.json`
- macOS: `~/Library/Application Support/com.icarus.graph-explorer/argument-workspace/library-v2.json`
- Linux: `$XDG_DATA_HOME` (or `~/.local/share`) followed by
  `com.icarus.graph-explorer/argument-workspace/library-v2.json`
...
```powershell
$env:ICARUS_ARGUMENT_LIBRARY_PATH = Join-Path $env:LOCALAPPDATA 'com.icarus.graph-explorer\argument-workspace\library-v2.json'
```
...
temporary schema-v2 JSON file before starting Inspector. Never commit or paste
